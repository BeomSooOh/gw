...
/*김석환 추가
 * 2012.06.20
 *  resetSelection(rowid) 함수 버그로 인하여 수정함.
 * 
 * */
		...
		firstr = "<tr class='jqgfirstrow' role='row' style='height:auto'>";
            ...
            $("th", thr).each(function(j) {
                ...
                if (ts.p.colModel[j].resizable) {
                    ...
                    if (!$.browser.opera) { $(res).css("cursor", ""); }
        ...
        resetSelection: function(rowid) {
            return this.each(function() {
                ...
                if (typeof (rowid) !== "undefined") {
                	/*김석환 추가 시작 
                	 * resetSelection 될때마다 t.p.selarrrow 배열에서도 해당 rowid 제거 해야함
                	 * */
                	
                	if(t.p.selarrrow){
	                	var tempArr__ = [];
	                	for(var t_index = 0;t_index<t.p.selarrrow.length; t_index++){
	                		if(t.p.selarrrow[t_index] && rowid!=t.p.selarrrow[t_index]){
	                			tempArr__[tempArr__.length] = t.p.selarrrow[t_index];
	                		}
	                	}
	                	t.p.selarrrow = tempArr__;
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("span.ui-jqgrid-resize").each(function () {
					...
					if ($parent.is(":visible")) {
						this.style.cssText = 'height: ' + $parent.height() + 'px !important; cursor: ;';
					}
				});
