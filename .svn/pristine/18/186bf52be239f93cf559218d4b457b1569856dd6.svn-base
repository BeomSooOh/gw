/**************************************************
   ...
   20180315 -  팝업 풋터수정 공지 다시보지않음 ie오류등 (이준혁)
   20180413 -  팝업 하단 고정/유동 수정 회계오류 (이준혁)
   20180511 -  투명스크롤 추가 (이준혁)
   20180614 -  클라우드 컨텐츠 영역 높이 계산 추가 (이정미)
   20181005 -  horResize() 소스변경 :: 특정 고객사가 대응 (허정명)
   20181214 -  no_foot 클래스시 pop_foot 넓이 auto로 수정 (이준혁)
**************************************************/
    ...
    $(document).ready(function () {
        ...
        $(".btn_Detail").click(function () {
           // $(".SearchDetail").slideToggle("slow");
			...
			if(!$(".SearchDetail").hasClass("on")){
				$(".SearchDetail").slideDown("slow");
				$(".SearchDetail").addClass("on");
				//flex_size();
			} else {
				$(".SearchDetail").removeClass("on");
				$(".SearchDetail").slideUp("slow");
				//flex_size();
			}
			flex_size();
    ...
    function docHeaderResize() {
        var docHeaderHdi = $('.pop_sign_head').height();
		 setTimeout(function(){$('.pop_sign_con').css("paddingTop", $('.pop_sign_head').height() + "px")},1000);
   ...
   // 기록물철별문서
    function ta_size() {
		setTimeout(function(){
			...
			$(".sub_con").width(scw_wid - record_wid)
			},500);
    }
   ...
   //서식목록
    function npDocList(){
        ...
        if($(".sub_contents, .freeb_sub_contents").length > 0 ){
            var Cls = $(".sub_contents, .freeb_sub_contents");

            if(Cls.hasClass() == "sub_contents"){
            	scw_hei = $(".sub_contents").height();
            }else{
         		scw_hei = $(".freeb_sub_contents").css("height","100%");
            }
		}else{
            scw_hei = $("body").height();
		};
    ...
    */
    ...
    function note_size() {
        ...
        var sc_hei = $(".sub_contents, .freeb_sub_contents").height(conWrapHei - headWrapHei - footWrapHei);

        var sc_wid = $(".sub_contents, .freeb_sub_contents").width();
    ...
    var scW = $('.sub_contents, .freeb_sub_contents').width()-40;
    var scH = $('.sub_contents, .freeb_sub_contents').height()-20;
    ...
    */
    ...
    function adminWorkResizeBox() {
        var scW = $('.sub_contents, .freeb_sub_contents').width() - 40;
        var scH = $('.sub_contents, .freeb_sub_contents').height();
        var stwH = $('.sub_title_wrap').height();
    ...
    //근태현황관리 :: 20170529 (허정명)
    function dalResize() {
        var subHei = $('.sub_contents, .freeb_sub_contents').height();

        /* 개발적용 시 풀어야 함 (퍼블리싱 확인할 때 주석처리) */
        if ($(".sub_contents, .freeb_sub_contents").size() == 0) {
            subHei = $('body').height();
        }
    ...
    ********************************************************************************************************************/
    ...
    $(document).ready(function () {
        ...
        function horResize() {
            ...
            if($(".header_wrap") != null && $(".header_wrap").length > 0 && $("#oneContents").length == 0){
    ...
    function subConWid() {
        ...
        if ($(".side_wrap").length > 0) {
            //$(".sub_contents, .freeb_sub_contents").width($(".sub_wrap").width()-8);
        } else {
            //$(".sub_contents, .freeb_sub_contents").width($(".sub_wrap").width()-0);
        }
    }
...
$(document).ready(function() {
         if($(".pop_wrap > .pop_foot.add_clo").length > 0){
             $(".pop_wrap").append("<div class='pop_foot_bg'></div>");
			$(".pop_wrap").append("<div class='pop_closeall_bg'></div>");
		} else if($(".pop_wrap > .pop_foot").length > 0){
			$(".pop_wrap").append($('.pop_foot_bg').length > 0 ? '' : "<div class='pop_foot_bg'></div>");
		}
		...
		{
			flex_size_pop_load() // 팝업 테이블 세로 리사이징 (최초 로드시에만)
		} else {

			flex_size(); // 페이지 테이블 세로 리사이징
		}
        ...
        $(window).resize(function() {
			...
			flex_size() // 팝업 및 페이지 테이블 세로 리사이징
        });
    });
...
function pb_btn(){
	...
	$(".pop_wrap.no_foot > .pop_foot").css("width","auto");
 ...
 $.fn.hasScrollBar = function() {
	...
	var pop_head_hei = $(".pop_head").outerHeight();
	var top_box_hei = $(".top_box").outerHeight();
	var btn_div_hei  = $(".btn_div").outerHeight();
	var rightHeader_hei  = $(".rightHeader").outerHeight();
	var pager_hei  = $(".PUDD-UI-pager").outerHeight();
	var sc_head_hei  = $(".sc_head").outerHeight();
	var pop_foot_hei = $(".pop_foot").outerHeight();
	var hw_hei = $(".header_wrap").outerHeight();
	var st_hei = $(".sub_title_wrap").outerHeight();
	//var pf_hei = p_foot;

	if ($(".SearchDetail").hasClass("on"))
	{
		if ($(".SearchDetail").children("dl").size() == 1)
		{
			var sd_hei = 55;
		} else if($(".SearchDetail").children("dl").size() == 2)
		{
			var sd_hei = 89;
		} else if($(".SearchDetail").children("dl").size() == 3)
		{
			var sd_hei = 123;
		} else if($(".SearchDetail").children("dl").size() == 4)
		{
			var sd_hei = 157;
		} else if($(".SearchDetail").children("dl").size() == 5)
		{
			var sd_hei = 191;
		} else if($(".SearchDetail").children("dl").size() == 6)
		{
			var sd_hei = 225;
		}
	}else {
	...
	var foot_hei = $(".foot").outerHeight();

	var fs_val = $(".flex_size").attr("value");

	if ($(".flex_size").length > 0)
		{
			$(".pop_foot").css("position","static");
			$(".pop_foot_bg").remove();
		}

	$(".flex_size").css("height", doc_hei - pop_head_hei - top_box_hei - btn_div_hei - rightHeader_hei - pager_hei - sc_head_hei - pop_foot_hei - hw_hei - st_hei - foot_hei  - sd_hei  - fs_val );
		// return false;
		}

	function	flex_size_pop_load(){//팝업시 로드에만
		if ($(".flex_size").length > 0){
			var mh = $(".flex_size").css("minHeight");
			$(".flex_size").css("height",mh);
			$(".pop_foot").css("position","static");
			$(".pop_foot_bg").remove();
		}
	}
...
function selectBox(obj){
	var This = $(obj);
	var el = $(".ui-selectmenu-open");
	var ThisY = This.offset().top;
	var bodyChk = $(window).height();
	var listHeight = el.height();
	var elemHeight = (isNaN(Number(This.height())) ? 0 : Number(This.height())) + 2;
	var bottomHeight = ( bodyChk - ( ThisY + elemHeight ) );

	if(bottomHeight < listHeight){
		el.css({"top": ( ThisY - listHeight ) ,"bottom":"inherit"});
	};
}
...
//투명스크롤
$(document).ready(function() {
	if ((document.documentElement.innerHTML || document.documentElement.innerText).indexOf('jquery.mCustomScrollbar.js') > -1) {
		comScroll();
    freebScroll()
	}
});
...
function comScroll(){
	$(".comScroll").mCustomScrollbar({axis:"yx",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
    $(".comScrollY").mCustomScrollbar({axis:"y",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
	$(".comScrollY_a").mCustomScrollbar({axis:"y",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:true});
    $(".comScrollX").mCustomScrollbar({axis:"x",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
    $(".comScrollX_dark").mCustomScrollbar({axis:"x",theme:"dark",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
};

function freebScroll(){
    $(".Scroll").mCustomScrollbar({axis:"yx",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
    $(".ScrollX").mCustomScrollbar({axis:"x",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
	...
	$(".ScrollY_a").mCustomScrollbar({axis:"y",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:true});
    $(".ScrollX_dark").mCustomScrollbar({axis:"x",theme:"dark",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
    $(".ScrollY_dark").mCustomScrollbar({axis:"y",theme:"dark",updateOnContentResize:true,updateOnImageLoad:true,updateOnSelectorChange:true,autoExpandScrollbar:false,moveDragger:true,autoHideScrollbar:false});
};

function protletScrollReset(){
    $(".ptl_content > .Scroll").mCustomScrollbar("destroy");
    $(".ptl_content > .ScrollY").mCustomScrollbar("destroy");
    $(".ptl_content > .ScrollX").mCustomScrollbar("destroy");
