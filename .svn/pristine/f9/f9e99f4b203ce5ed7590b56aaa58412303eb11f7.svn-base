	...
	<script type="text/javascript">
		...
		function req() {
    		...
    		$.ajax({
                ...
                dataType: "json",
    			data:{header:head, body:body, uri:uriStr},
    			success:function(d){
...
<div class="sub_contents_wrap" style="height:100%">
	...
	<div class="sub_contents_border" style="height:100%">
		...
		<div class="sub_left">
        	...
        	<h2><%=BizboxAMessage.getMessage("TX000017911","PC메신저 API")%></h2>
			</div> 
			<div id="treeview"></div>
		</div>		
		<div class="comp_sub_con" id="comp_info" style="margin-left:-1px;height:100%; width: 100%">
			...
			<div style="text-align:center;margin-top:10px">
				<button type="button" onclick="req()" style="width:100px;height:20px"><%=BizboxAMessage.getMessage("TX000003418","요청")%></button>
			</div>
		</div>
...
<script type="text/javascript"> 
    ...
    $("#treeview").kendoTreeView({
		...
		dataSource: [
		    {
				text: "<%=BizboxAMessage.getMessage("TX000017912","링크메뉴 버전")%>", expanded: true, spriteCssClass: "folder", urlPath:address+"/api/msg/MsgMenuVer" 
			},
			{
				text: "<%=BizboxAMessage.getMessage("TX000017913","링크메뉴 리스트")%>", expanded: true, spriteCssClass: "folder", urlPath:address+"/api/msg/MsgMenuList" 
			},
			{
				text: "SSO Token", expanded: true, spriteCssClass: "folder", urlPath:address+"/api/msg/MsgLinkToken" 
			}
		], 
		select:onSelect
	});
