...
<head>
    ...
    <script type="text/javascript" src="<c:url value='/js/neos/systemx/systemx.main.js?ver=20201021' />"></script>
     ...
     <!-- top js -->
    <script type="text/javascript" src="<c:url value='/js/Scripts/jquery.alsEN-1.0.min.js?ver=20201021' />"></script>
	...
	$(document).ready(function() {
		...
		if(popYn == "Y") {
			...
			if(urlGubun == "mail") {
				mainmenu.clickTopBtn(200000000,"<%=BizboxAMessage.getMessage("TX000000262","메일")%>", "<%=BizboxAProperties.getCustomProperty("BizboxA.Cust.EnpassMailUrl")%>", "mail");
			}
			else if(urlGubun == "edms") {
				mainmenu.clickTopBtn(500000000,"<%=BizboxAMessage.getMessage("TX000011134","게시판")%>", "", "edms");
			}
			else {
				mainmenu.clickTopBtn(100000000,"<%=BizboxAMessage.getMessage("TX000000479","전자결재")%>", '/edoc/EdocMain.do', "ea");
			}
		}
