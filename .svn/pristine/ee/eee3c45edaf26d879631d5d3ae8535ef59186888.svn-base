...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		<c:if test="${isMasterAuth == true && (firstGnbRenderTp == 'SSO' || firstGnbRenderTp == 'WEB')}">
		setTimeout("helpDeskAdminAlertPop();", 1500);
		</c:if>
	...
	function setAuthBtn(){
		...
		if("${userSe}" == "ADMIN" && "${topType}" != "mailex"){
			$("#btnUser").show();
			if("${isMasterAuth}" == "true"){
				$("#btnMaster").show();
			}
		...
		if("${userSe}" == "MASTER" && "${topType}" != "mailex"){
			$("#btnUser").show();
			if("${isAdminAuth}" == "true"){
				$("#btnAdmin").show();
			}
...
//	 				$("." + alertId).removeClass("unread");
		...
		$(document).ready(function() {
			...
			img.onload=function(){
				var innerHTML = "";
				if("${userSe}" == "USER")
					innerHTML = "<img src=\"${uploadPathBase}img/logo/${loginVO.groupSeq}/${loginVO.compSeq}/IMG_COMP_LOGO_${loginVO.compSeq}.${imgMap.file_extsn}\" onerror=\"this.src='/gw/Images/freeb/gnb_logo01<c:if test="${cloudYn == 'N'}">_pkg</c:if>.png'\" />";
				else
					innerHTML = "<img src=\"${uploadPathBase}img/logo/${loginVO.groupSeq}/${loginVO.compSeq}/IMG_COMP_LOGO_${loginVO.compSeq}.${imgMap.file_extsn}\" onerror=\"this.src='/gw/Images/freeb/gnb_logo01<c:if test="${cloudYn == 'N'}">_pkg</c:if>.png'\" />";
			...
			img.onerror=function(){
				var innerHTML = "";
				if("${userSe}" == "USER")
					innerHTML = "<img src=\"/gw/Images/freeb/gnb_logo01<c:if test="${cloudYn == 'N'}">_pkg</c:if>.png\" onerror=\"this.src='/gw/Images/freeb/gnb_logo01<c:if test="${cloudYn == 'N'}">_pkg</c:if>.png'\" />";
				else
					innerHTML = "<img src=\"${uploadPathBase}img/logo/${loginVO.groupSeq}/${loginVO.compSeq}/IMG_COMP_LOGO_${loginVO.compSeq}.${imgMap.file_extsn}\" onerror=\"this.src='/gw/Images/freeb/gnb_logo01<c:if test="${cloudYn == 'N'}">_pkg</c:if>.png'\" />";		
				$("#logoImgHyperLink").html(innerHTML);
			} 
			img.src='${uploadPathBase}img/logo/${loginVO.groupSeq}/${loginVO.compSeq}/IMG_COMP_LOGO_${loginVO.compSeq}.${imgMap.file_extsn}';		
...
<div class="header_sub_wrap">
	<div class="header_sub">
		<div class="freeb_logo">
			<c:if test="${topType == 'mailex'}"> <!-- 메일전용 계정 추가 -->
				<h1><a href="<c:url value='/bizboxMailEx.do' />" id="logoImgHyperLink"></a></h1>
			</c:if>
			<c:if test="${topType != 'mailex'}"> <!-- 메일전용 계정 추가 -->
				<h1><a href="<c:url value='${mainPage}' />" id="logoImgHyperLink"></a></h1>
			</c:if>		
		</div>
		<!-- user misc -->
		<div class="misc">
			...
			<div class="profile_wrap">
				...
				<!-- 팝업::프로필 --> 
				<div class="header_pop_wrap profile_comp_box" style="display:none;z-index:999">  
					<div class="h_pop_box" id="profileInfo" tabindex="1" style="outline: none;"> 
						...
						<div class="bot">
							<div class="btn_cen pt12">
								...
								<input type="button" value="<%=BizboxAMessage.getMessage("TX000002980","로그아웃")%>" onclick="cloudLogout();"/>
							</div>
						</div>
						</c:if>
...
<!-- 											<img id="alertSpinImg" src="/gw/Images/ajax-loader.gif" style="width: 20px;"> -->
			...
			<c:if test="${userSe == 'USER' && topType != 'mailex'}"> <!-- 메일전용 계정 추가 -->
				 ...
				 <a href="#" id="btnAdmin" style="display:none;" onclick="changeMode('ADMIN')" class="fn_btn" title="<%=BizboxAMessage.getMessage("TX000000705","관리자")%>"><%=BizboxAMessage.getMessage("TX000000705","관리자")%></a>
				 </c:if>
				 <c:if test="${isMasterAuth == true}"> 
				 <a href="#" id="btnMaster" style="display:none;" onclick="changeMode('MASTER')" class="fn_btn" title="<%=BizboxAMessage.getMessage("TX000016047","마스터")%>"><%=BizboxAMessage.getMessage("TX000016047","마스터")%></a>
				 </c:if>
				 </div>  
			</c:if>		
			...
			<c:if test="${userSe == 'ADMIN' && topType != 'mailex'}">  <!-- 메일전용 계정 추가 -->
				 <div class="">  
				 <a href="#" id="btnUser" style="display:none;" onclick="changeMode('USER')" class="fn_btn" title="<%=BizboxAMessage.getMessage("TX000000286","사용자")%>"><%=BizboxAMessage.getMessage("TX000000286","사용자")%></a>
				 <c:if test="${isMasterAuth == true}"> 
				 <a href="#" id="btnMaster" style="display:none;" onclick="changeMode('MASTER')" class="fn_btn" title="<%=BizboxAMessage.getMessage("TX000016047","마스터")%>"><%=BizboxAMessage.getMessage("TX000016047","마스터")%></a>
				 </c:if>
				 </div>  
...
<c:if test="${type != 'cloudSub'}">
<div class="freeb_menu"><!-- 메뉴 -->
<div class="freeb_menu_in" style="display:none;">
		...
		<tr>
			<c:if test="${topType ne 'mailex'}"> <!-- 메일전용 계정 추가 -->
				<c:forEach items="${topMenuList}" var="list" varStatus="c">
					<td>
						<div class="fm_div fm_${list.menuImgClass}" onclick="onclickTopCustomMenu(${list.menuNo},'${list.name }', '${list.urlPath}', '${list.urlGubun}', '${list.lnbMenuNo}', '${list.ssoUseYn}');" id="topMenu${list.menuNo}" name="topMenu">
							<div class="fm_div_in">
								<div class="ico"></div>
...
<div class="freeb_more" moreBtn="Y">
	<div class="fm_div plus" moreBtn="Y">
		<div class="fm_div_in" moreBtn="Y">
			...
			<div class="txt" moreBtn="Y"><%=BizboxAMessage.getMessage("TX000006348","더보기")%></div>
		</div>
	</div>
</div>
<div class="freeb_more_box">
...
<form id="formSearch" name="formSearch" action="totalSearch.do?t=dfds" method="post">  
