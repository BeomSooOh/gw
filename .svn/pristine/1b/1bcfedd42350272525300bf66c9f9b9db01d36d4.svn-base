...
public class KISA_SEED_CBC {
	...
	private static int endian = Common.BIGENDIAN;
	...
	private static final void SeedRound(int[] t, int lr[], int l0, int l1, int r0, int r1, int[] k, int kOffset) {
	...
	private static final void RoundKeyUpdate0(int t[], int[] k, int kOffset, int abcd[], int kc) {
		t[0] = abcd[ABCD_A] + abcd[ABCD_C] - kc;
		t[1] = abcd[ABCD_B] + kc - abcd[ABCD_D];
		k[kOffset+0] = SS0[GetB0(t[0])&0x0ff] ^ SS1[GetB1(t[0])&0x0ff] ^ SS2[GetB2(t[0])&0x0ff] ^ SS3[GetB3(t[0])&0x0ff];
		k[kOffset+1] = SS0[GetB0(t[1])&0x0ff] ^ SS1[GetB1(t[1])&0x0ff] ^ SS2[GetB2(t[1])&0x0ff] ^ SS3[GetB3(t[1])&0x0ff];
		t[0] = abcd[ABCD_A]; 
		abcd[ABCD_A] = ((abcd[ABCD_A]>>8)&0x00ffffff) ^ (abcd[ABCD_B]<<24);
		abcd[ABCD_B] = ((abcd[ABCD_B]>>8)&0x00ffffff) ^ (t[0]<<24);
	}

	private static final void RoundKeyUpdate1(int t[], int []k, int kOffset, int abcd[], int kc) {
		t[0] = abcd[ABCD_A] + abcd[ABCD_C] - kc;
		t[1] = abcd[ABCD_B] + kc - abcd[ABCD_D];
		k[kOffset+0] = SS0[GetB0(t[0])&0x0ff] ^ SS1[GetB1(t[0])&0x0ff] ^ SS2[GetB2(t[0])&0x0ff] ^ SS3[GetB3(t[0])&0x0ff];
		k[kOffset+1] = SS0[GetB0(t[1])&0x0ff] ^ SS1[GetB1(t[1])&0x0ff] ^ SS2[GetB2(t[1])&0x0ff] ^ SS3[GetB3(t[1])&0x0ff];
	...
	private static void BLOCK_XOR_CBC(int[] outValue, int outValueOffset, int[] inValue1, int inValueOffset, int[] inValue2, int inValue2Offset) {
		outValue[outValueOffset+0] = (inValueOffset<inValue1.length?inValue1[inValueOffset+0]:0) ^ (inValue2Offset<inValue2.length?inValue2[inValue2Offset+0]:0);
		outValue[outValueOffset+1] = (inValueOffset+1<inValue1.length?inValue1[inValueOffset+1]:0) ^ (inValue2Offset+1<inValue2.length?inValue2[inValue2Offset+1]:0);
		outValue[outValueOffset+2] = (inValueOffset+2<inValue1.length?inValue1[inValueOffset+2]:0) ^ (inValue2Offset+2<inValue2.length?inValue2[inValue2Offset+2]:0);
		outValue[outValueOffset+3] = (inValueOffset+3<inValue1.length?inValue1[inValueOffset+3]:0) ^ (inValue2Offset+3<inValue2.length?inValue2[inValue2Offset+3]:0);
	}
	...
	private static void KISA_SEED_Encrypt_Block_forCBC( int[] in, int inOffset, int[] out, int outOffset, KISA_SEED_KEY ks ) {
		...
		lr[LR_L0] = in[inOffset+0];
		lr[LR_L1] = in[inOffset+1];
		lr[LR_R0] = in[inOffset+2];
		lr[LR_R1] = in[inOffset+3];

		// Reorder for big endian
		// Because SEED use little endian order in default
		if(Common.BIGENDIAN != endian) {
		...
		out[outOffset+0] = lr[LR_R0];
		out[outOffset+1] = lr[LR_R1];
		out[outOffset+2] = lr[LR_L0];
		out[outOffset+3] = lr[LR_L1];

	}

	private static void KISA_SEED_Decrypt_Block_forCBC( int[] in, int inOffset, int[] out, int outOffset, KISA_SEED_KEY ks ) {
		int lr[] = new int[4];				// Iuput/output values at each rounds
		int t[] = new int[2];				// Temporary variables for round function F
		int k[] = ks.key_data;				// Pointer of round keys
	...
	public static int SEED_CBC_Process( KISA_SEED_INFO pInfo, int[] in, int inLen, int[] out, int[] outLen ) {
		...
		if( KISA_ENC_DEC.KISAENCRYPT == pInfo.encrypt ) {
			pdwXOR = pInfo.ivec;
			inOffset = 0;
			outOffset = 0;
			pdwXOROffset = 0;
			...
			{
				BLOCK_XOR_CBC( out, outOffset, in, inOffset, pdwXOR, pdwXOROffset );

				KISA_SEED_Encrypt_Block_forCBC( out, outOffset, out, outOffset, pInfo.seed_key );

				pdwXOR = out;
				pdwXOROffset = outOffset;

				nCurrentCount += BLOCK_SIZE_SEED;
				inOffset += BLOCK_SIZE_SEED_INT;		
				outOffset += BLOCK_SIZE_SEED_INT;		
			}

			outLen[0] = nCurrentCount - BLOCK_SIZE_SEED; 
			pInfo.buffer_length = (inLen - outLen[0]);

			Common.memcpy( pInfo.ivec, pdwXOR, pdwXOROffset, BLOCK_SIZE_SEED );
			Common.memcpy( pInfo.cbc_buffer, in, inOffset, pInfo.buffer_length );
		}
		else {
			pdwXOR = pInfo.ivec;
			inOffset = 0;
			...
			while( nCurrentCount <= inLen )			
			{
				KISA_SEED_Decrypt_Block_forCBC( in, inOffset, out, outOffset, pInfo.seed_key );

				BLOCK_XOR_CBC( out, outOffset, out, outOffset, pdwXOR, pdwXOROffset );

				pdwXOR = in;
				pdwXOROffset = inOffset;
			...
			Common.memcpy( pInfo.ivec, pdwXOR, pdwXOROffset, BLOCK_SIZE_SEED );		 	
			Common.memcpy( pInfo.cbc_last_block, out, outOffset-BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED );	
	...
	public static int SEED_CBC_Close( KISA_SEED_INFO pInfo, int[] out, int outOffset, int[] outLen ) {
		int nPaddngLeng;
		int i;
		...
		if( KISA_ENC_DEC.KISAENCRYPT == pInfo.encrypt ) {
			...
			KISA_SEED_Encrypt_Block_forCBC( pInfo.cbc_buffer, 0, out, outOffset, pInfo.seed_key );
			...
			if( nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED )   
			{
				...
				{
					Common.set_byte_for_int(out, outOffset-i, (byte)0x00, endian);
				}
	...
	public static byte[] SEED_CBC_Encrypt( byte[] pbszUserKey, byte[] pbszIV, byte[] message, int messageOffset, int messageLength ) {
		...
		byte[] pbszPlainText = new byte[messageLength];
		System.arraycopy(message, messageOffset, pbszPlainText, 0, messageLength);
	...
	public static byte[] SEED_CBC_Decrypt( byte[] pbszUserKey, byte[] pbszIV, byte[] message, int messageOffset, int messageLength ) {
		KISA_SEED_INFO info = new KISA_SEED_INFO();
		int[] outbuf;
		int[] data;
		byte[] cdata;
		...
		byte[] pbszCipherText = new byte[messageLength];
		System.arraycopy(message, messageOffset, pbszCipherText, 0, messageLength);
		...
		if( SEED_CBC_Close( info, outbuf, (nRetOutLeng[0]), nPaddingLeng ) == 1 )
		{
			...
			int pdmessageLength = nRetOutLeng[0]-nPaddingLeng[0];
			byte[] result = new byte[pdmessageLength];
			System.arraycopy(pbszPlainText, 0, result, 0, pdmessageLength);
	...
	public static final class KISA_ENC_DEC {
		public static final int KISADECRYPT = 0;
		public static final int KISAENCRYPT = 1;
		...
		public static final KISA_ENC_DEC KISA_ENCRYPT = new KISA_ENC_DEC(KISAENCRYPT);
		public static final KISA_ENC_DEC KISA_DECRYPT = new KISA_ENC_DEC(KISADECRYPT);
	...
	public static class Common {
		...
		public static void arraycopy_offset(byte[] dst, int dstOffset, byte[] src, int srcOffset, int length) {
			for(int i=0; i<length; i++) {
		...
		public static void arrayinit_offset(byte[] dst, int dstOffset, byte value, int length) {
			for(int i=0; i<length; i++) {
		...
		public static void memcpy(int[] dst, int[] src, int srcOffset, int length) {
		...
		public static void set_byte_for_int(int[] dst, int bOffset, byte value, int endian) {
			if(endian == BIGENDIAN) {
				int shiftValue = (3-bOffset%4)*8;
				int maskValue =  0x0ff << shiftValue;
				int maskValue2 = ~maskValue;
				int value2 = (value&0x0ff) << shiftValue;
				dst[bOffset/4] = (dst[bOffset/4] & maskValue2) | (value2 & maskValue);    
			} else {
				int shiftValue = (bOffset%4)*8;
				int maskValue =  0x0ff << shiftValue;
				int maskValue2 = ~maskValue;
				int value2 = (value&0x0ff) << shiftValue;
				dst[bOffset/4] = (dst[bOffset/4] & maskValue2) | (value2 & maskValue);    
		...
		public static byte get_byte_for_int(int[] src, int bOffset, int endian) {
			if(endian == BIGENDIAN) {
				int shiftValue = (3-bOffset%4)*8;
				int maskValue =  0x0ff << shiftValue;
				int value = (src[bOffset/4] & maskValue) >> shiftValue;
				return (byte)value;
			} else {
				int shiftValue = (bOffset%4)*8;
				int maskValue =  0x0ff << shiftValue;
		...
		public static void byte_to_int(int[] dst, int dstOffset, byte[] src, int srcOffset, int endian) {
			if(endian == BIGENDIAN) {
				dst[dstOffset] = ((0x0ff&src[srcOffset]) << 24) | ((0x0ff&src[srcOffset+1]) << 16) | ((0x0ff&src[srcOffset+2]) << 8) | (0x0ff&src[srcOffset+3]);
			} else {
				dst[dstOffset] = (0x0ff&src[srcOffset]) | ((0x0ff&src[srcOffset+1]) << 8) | ((0x0ff&src[srcOffset+2]) << 16) | ((0x0ff&src[srcOffset+3]) << 24);
			}
		}
		
		public static int byte_to_int(byte[] src, int srcOffset, int endian) {
			if(endian == BIGENDIAN) {
				return ((0x0ff&src[srcOffset]) << 24) | ((0x0ff&src[srcOffset+1]) << 16) | ((0x0ff&src[srcOffset+2]) << 8) | (0x0ff&src[srcOffset+3]);
			} else {
				return (0x0ff&src[srcOffset]) | ((0x0ff&src[srcOffset+1]) << 8) | ((0x0ff&src[srcOffset+2]) << 16) | ((0x0ff&src[srcOffset+3]) << 24);
			}
		}

		public static int byte_to_int_big_endian(byte[] src, int srcOffset) {
			return ((0x0ff&src[srcOffset]) << 24) | ((0x0ff&src[srcOffset+1]) << 16) | ((0x0ff&src[srcOffset+2]) << 8) | (0x0ff&src[srcOffset+3]);
		}

		public static void int_to_byte(byte[] dst, int dstOffset, int[] src, int srcOffset, int endian) {
			int_to_byte_unit(dst, dstOffset, src[srcOffset], endian);
		}
		
		public static void int_to_byte_unit(byte[] dst, int dstOffset, int src, int endian) {
			if(endian == BIGENDIAN) {
				dst[dstOffset] = (byte)((src>> 24) & 0x0ff);
				dst[dstOffset+1] = (byte)((src >> 16) & 0x0ff);
				dst[dstOffset+2] = (byte)((src >> 8) & 0x0ff);
				dst[dstOffset+3] = (byte)((src) & 0x0ff);
			} else {
				dst[dstOffset] = (byte)((src) & 0x0ff);
				dst[dstOffset+1] = (byte)((src >> 8) & 0x0ff);
				dst[dstOffset+2] = (byte)((src >> 16) & 0x0ff);
				dst[dstOffset+3] = (byte)((src >> 24) & 0x0ff);
			}
			
		}

		public static void int_to_byte_unit_big_endian(byte[] dst, int dstOffset, int src) {
			dst[dstOffset] = (byte)((src>> 24) & 0x0ff);
			dst[dstOffset+1] = (byte)((src >> 16) & 0x0ff);
			dst[dstOffset+2] = (byte)((src >> 8) & 0x0ff);
			dst[dstOffset+3] = (byte)((src) & 0x0ff);
		...
		//1블럭(128비트 XOR)
		public static void BLOCK_XOR_PROPOSAL(int[] outValue, int outValueOffset, int[] inValue1, int inValue1Offset, int[] inValue2, int inValue2Offset) {
			outValue[outValueOffset+0] = (inValue1Offset<inValue1.length?inValue1[inValue1Offset+0]:0) ^ (inValue2Offset<inValue2.length?inValue2[inValue2Offset+0]:0);
			outValue[outValueOffset+1] = (inValue1Offset+1<inValue1.length?inValue1[inValue1Offset+1]:0) ^ (inValue2Offset+1<inValue2.length?inValue2[inValue2Offset+1]:0);
			outValue[outValueOffset+2] = (inValue1Offset+2<inValue1.length?inValue1[inValue1Offset+2]:0) ^ (inValue2Offset+2<inValue2.length?inValue2[inValue2Offset+2]:0);
			outValue[outValueOffset+3] = (inValue1Offset+3<inValue1.length?inValue1[inValue1Offset+3]:0) ^ (inValue2Offset+3<inValue2.length?inValue2[inValue2Offset+3]:0);
		}
	}
...
//		};
//		
//	    int plainTextLength = 14;
//	    int ciphertextLength = 16;		
//	    
//	    byte[] defaultCipherText = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData, 0, plainTextLength);
//	    
//	    SEED_CBC_Decrypt(pbUserKey, bszIV, defaultCipherText, 0, ciphertextLength);
//
//	    byte[] cipher1 = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData1,0, 2);
//	    
//	    SEED_CBC_Decrypt(pbUserKey, bszIV, cipher1, 0, 16);
//
//	    byte[] cipher2 = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData2,0, 16);
//	    
//	    SEED_CBC_Decrypt(pbUserKey, bszIV, cipher2, 0, 32);
//	    
//	    byte[] cipher3 = SEED_CBC_Encrypt(pbUserKey, bszIV, pbData3, 0, 18);
//	    
//	    SEED_CBC_Decrypt(pbUserKey, bszIV, cipher3, 0, 32);
//	    
//	    plainTextLength = 14;
//	    
//	    KISA_SEED_INFO info = new KISA_SEED_INFO();
...
//		
//
//		int nPlainTextPadding = BLOCK_SIZE_SEED - (plainTextLength % BLOCK_SIZE_SEED);
//		byte []newpbszPlainText = new byte[plainTextLength+nPlainTextPadding];
//		Common.arraycopy(newpbszPlainText, pbData, plainTextLength);		
//		
//		
//		byte[] pbszCipherText = new byte[newpbszPlainText.length];			
//		
...
//		
//		int processBlockLeng = BLOCK_SIZE_SEED * 2;								//한번에 처리할 BLOCK
//		
//		int[] outbuf = new int[processBlockLeng/4];
//		
//		for(j=0; j<plainTextLength-processBlockLeng; )
//		{			
//			System.arraycopy(pbData, j, newpbszPlainText, 0, processBlockLeng);
//			data = chartoint32_for_SEED_CBC(newpbszPlainText, processBlockLeng);			
//			SEED_CBC_Process( info, data, processBlockLeng, outbuf, nRetOutLeng );			
//			cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0]);			
//			System.arraycopy(cdata, 0, pbszCipherText, j, nRetOutLeng[0]);			
//			j += nRetOutLeng[0];
//		}
//		
//		int remainleng = plainTextLength % processBlockLeng;
//		if(remainleng == 0)
//		{
//			remainleng = processBlockLeng;
//		}
...
//	    ciphertextLength = 16;
//	    
//	    int pbszCipherTextOffset = 0;
//    
//	    
//	    byte[] message = new byte[ciphertextLength];
//		System.arraycopy(pbszCipherText, pbszCipherTextOffset, message, 0, ciphertextLength);
//		
...
//		
//		processBlockLeng = BLOCK_SIZE_SEED * 2;
//		
//
//		outbuf = new int[processBlockLeng/4];
//		
//		byte []newpbszCipherText = new byte[nCipherTextLen];
//		byte []pbszPlainText = new byte[nCipherTextLen];
//		
//		
//		for(j=0; j<nCipherTextLen - processBlockLeng; )
//		{
//			System.arraycopy(message, j, newpbszCipherText, 0, processBlockLeng);
//			data = chartoint32_for_SEED_CBC(newpbszCipherText, processBlockLeng);	
//			SEED_CBC_Process( info, data, processBlockLeng, outbuf, nRetOutLeng );			
//			cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng[0]);			
...
//		}
//		
//		remainleng =  nCipherTextLen%processBlockLeng;
//		if(remainleng == 0)
//		{
//			remainleng = processBlockLeng;
//		}
...
//			
//			int messageLength = remainleng-nPaddingLeng[0];
//			
//			result = new byte[messageLength];
//			System.arraycopy(newpbszPlainTexts, 0, result, 0, messageLength);
//			
//			data = null;
//			cdata = null;
//			outbuf = null;
...
//	    
//
//		nPlainTextPadding = BLOCK_SIZE_SEED - (plainTextLength % BLOCK_SIZE_SEED);
//		newpbszPlainText = new byte[plainTextLength+nPlainTextPadding];
//		Common.arraycopy(newpbszPlainText, pbData1, plainTextLength);		
//		
//		
...
//		SEED_CBC_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbUserKey, bszIV );
//		
//		processBlockLeng = BLOCK_SIZE_SEED * 2;								//한번에 처리할 BLOCK
//		
//
//		outbuf = new int[processBlockLeng/4];
//		pbszPlainText = new byte[processBlockLeng];
//		
//		for(j=0; j<plainTextLength-processBlockLeng; )
//		{			
//			System.arraycopy(pbData1, j, newpbszPlainText, 0, processBlockLeng);
//			data = chartoint32_for_SEED_CBC(newpbszPlainText, processBlockLeng);
//			
//			SEED_CBC_Process( info, data, processBlockLeng, outbuf, nRetOutLeng );
//			
...
//		}
//		
//		remainleng = plainTextLength % processBlockLeng;
//		if(remainleng == 0)
//		{
//			remainleng = processBlockLeng;
//		}
...
//	    ciphertextLength = 16;
//	    
//	    pbszCipherTextOffset = 0;
//    
//	    
//	    message = new byte[ciphertextLength];
//		System.arraycopy(pbszCipherText, pbszCipherTextOffset, message, 0, ciphertextLength);
//		
...
//		nPlainTextPadding = BLOCK_SIZE_SEED - (plainTextLength % BLOCK_SIZE_SEED);
//		newpbszPlainText = new byte[plainTextLength+nPlainTextPadding];
//		Common.arraycopy(newpbszPlainText, pbData2, plainTextLength);		
//		pbszCipherText = new byte[newpbszPlainText.length];			
//		SEED_CBC_init( info, KISA_ENC_DEC.KISA_ENCRYPT, pbUserKey, bszIV );
//		processBlockLeng = BLOCK_SIZE_SEED * 2;								//한번에 처리할 BLOCK
//		outbuf = new int[processBlockLeng/4];
...
//		for(j=0; j<plainTextLength-processBlockLeng; )
//		{			
//			System.arraycopy(pbData2, j, newpbszPlainText, 0, processBlockLeng);
//			data = chartoint32_for_SEED_CBC(newpbszPlainText, processBlockLeng);
//			
//			SEED_CBC_Process( info, data, processBlockLeng, outbuf, nRetOutLeng );
//			
...
//		nPlainTextPadding = BLOCK_SIZE_SEED - (plainTextLength % BLOCK_SIZE_SEED);
//		newpbszPlainText = new byte[plainTextLength+nPlainTextPadding];
//		Common.arraycopy(newpbszPlainText, pbData3, plainTextLength);		
//		
//		
//		pbszCipherText = new byte[newpbszPlainText.length];			
//		
...
//		for(j=0; j<plainTextLength-processBlockLeng; )
//		{			
//			System.arraycopy(pbData3, j, newpbszPlainText, 0, processBlockLeng);
//			data = chartoint32_for_SEED_CBC(newpbszPlainText, processBlockLeng);
//			
//			SEED_CBC_Process( info, data, processBlockLeng, outbuf, nRetOutLeng );
//			
