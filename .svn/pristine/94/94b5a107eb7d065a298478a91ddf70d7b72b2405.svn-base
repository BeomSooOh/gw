...
/**
 *수정일자 : 2019-03-20 배성원 
 *수정사유 : 전자결재 날짜 표현 방식 변경에 따른 sepType(구분자 인자 추가) 2019.03.20 > 2019-03-20
 *이름 : ncCom_Date()
 *설명 : 날짜를 리턴한다
 *인자[argDate] : 날짜 
 *인자[argFlag] : 날짜형태 '/','-', '.'
 ...
 *리턴 : 날짜형태
*/
function ncCom_Date(argDate, argFlag, sepType){
	...
	var day ="";
	var separator = (sepType == undefined) ? "." : sepType;
	...
	switch (argFlag) {
		...
		case "M" : day = m;  break;
		case "D" : day = d;  break;
		case "YM" : day = y + separator + m ; break;
		case "SYMD": day = sy + separator + m + separator + d; break ;
...
function checkAll(checks, isCheck){
	...
	/*포틀릿 전자결재(비) 문서전체창 시 하단 여백 제거*/
	function openWindow3(url,  windowName, width, height, strScroll, strResize ){

		var pop = "" ;
		var intHeight = screen.height - 250;
		...
		if(windowName == "_blank"){
			...
			pop = window.open(url, windowName, "width=" + width + ", height=" + intHeight + ", top="+ intTop +", left="+ windowX +", scrollbars="+ strScroll+", resizable="+ strResize);	
		}
	...
	function checkLength(maxlen, obj) {
		    ...
		    if (len == 0) {
		        ...
		        for(var k=0; k<len; k++) {
		            ...
		            if(msglen < 0) {
		               alert(NeosUtil.getMessage("TX000016545","영문은 {0}자, 한글은 {1}자 까지 입력할 수 있습니다.").replace("{0}",maxlen*2).replace("{1}",maxlen));
		                obj.value = txt;
		                break;
		            } else {
		                txt += temp;
    ...
    function neosPopup(popType, param, popName) {
    	switch (popType) {
    		...
    		case 'POP_DOCEDIT' : //상신문서수정
    			...
    			}else if(popType == "POP_ONE_DOCVIEW_MAIN") {
    				param= "multiViewYN=N&"+param;
    				uri = "/ea/edoc/eapproval/docCommonDraftView.do?"+ param;
    			} else {
    				param= "multiViewYN=Y&"+param;
    			}
				
				return openWindow3(uri,  popName,  _g_aproval_width_, _g_aproval_heigth_, 1,1) ;
				break;
...
function setMailId(mailUrl, msg, params){
	var url = _g_contextPath_+'/cmm/systemx/changeEmailData.do';
	var result = null;
	/*var params = {};
	...
	$.ajax({
        ...
        error:function(request,status,error){
            ...
            alert(NeosUtil.getMessage("TX000016546","메일 ID변경시 오류가 발생하였습니다.　관리자에게 문의해주시기 바랍니다.").replace("　","\n")); 
        	result = null;
            state(0);
        }
    });
	...
	if(result){
		if(result.resultCode){
			if(result.resultCode == '0'){
				console.log("data.result.resultCode : "+result.resultCode);
				return 0;
			}else if(result.resultCode == 'UC0001'){
				
				return 1;
			}else if(result.resultCode == 'UC0002'){
				
				return 2;
			}else if(result.resultCode == 'UC0003'){
				
				return 3;
			}else if(result.resultCode == 'UC0004'){
...
function layerPopOpen(url, param, title, width, height, opener, parentId, childrenId) {
		...
		var $children;		//레이어 div
     	...
     	if (opener == "1") {
			...
			if (opener == "1" || opener == "2") {
				...
				$children = $parent.find("#"+parentId);8
				childrenId = ""; 
			}else {
 			$children = $("#" + childrenId);
			}
			...
			header += '<h1>' + title + '</h1>';
			header += '<a href="javascript:layerPopClose(\'' + childrenId + '\');" class="clo" style="display:block;"><img src="'+getContextPath()+'/Images/btn/btn_pop_clo02.png" alt="" /></a>';
			header += '</div>';
...
function state(state){
    ...
    var width = $(".pop_wrap").width() / 2;
    var height = $(".pop_wrap").height() / 2;
    ...
    if(state){
        ...
        //$('#dialog-form-background').css({'width':maskWidth,'height':maskHeight});  

        //애니메이션 효과
        $('#dialog-form-background').show();
        $('#dialog-form-background').fadeIn(1000);      
        $('#dialog-form-background').fadeTo("slow",0.8);   
    }else{
...
//사용자프로필팝업
function openEmpProfileInfoPop(compSeq, deptSeq, empSeq) {
	
	var param = "compSeq="+compSeq + "&deptSeq=" + deptSeq + "&empSeq=" + empSeq;
	
	 openWindow2('/gw/empProfileInfo.do' + "?" + param, 'empProfileInfoPop', 600, 380, 0, 1);
...
var getMessageJson = [
				...
				{"CD" : 'TX000007942', "KR" : '분류선택', "EN" : 'Selection of classification', "JP" : '分類選択', "CN" : '选择分类'},
				...
				{"CD" : 'TX000009208', "KR" : '부분공개비공개 여부는 구분번호를 선택해야 합니다', "EN" : 'The disclosure, nondisclosure of part or not should select the classification number.', "JP" : '', "CN" : ''},
				...
				{"CD" : 'TX000009241', "KR" : '문서는 1개만 선택해주세요', "EN" : 'Please select only one document.', "JP" : '', "CN" : ''},
				...
				{"CD" : 'TX000009430', "KR" : '등록되지 않은 양식입니다. 관리자에게 문의 바랍니다', "EN" : 'It is not the registered form. Please contact the manager. ', "JP" : '', "CN" : ''},
				...
				{"CD" : 'TX000011218', "KR" : '팝업을 해제해 주세요', "EN" : 'Please release the pop up.', "JP" : '', "CN" : ''},
				...
				{"CD" : 'TX000016571', "KR" : '비공개대상정보설명', "EN" : 'Explanation for the information of nondisclosure object. ', "JP" : '', "CN" : ''},
				...
				{"CD" : 'TX000017813', "KR" : 'ERP사원정보가 없습니다. 관리자에게 문의 바랍니다', "EN" : '', "JP" : '', "CN" : ''},
				{"CD" : 'TX000017814', "KR" : 'ERP사원정보가 올바르지 않습니다. 관리자에게 문의 바랍니다.', "EN" : '', "JP" : '', "CN" : ''},
				{"CD" : 'TX000017824', "KR" : '명세서등록 금액', "EN" : '', "JP" : '', "CN" : ''},
				{"CD" : 'TX000010608', "KR" : '데이터가 존재하지 않습니다', "EN" : 'Data don’t exist', "JP" : '', "CN" : ''},
				{"CD" : 'TX000019709', "KR" : '도움말', "EN" : 'The help files', "JP" : 'ヘルプ', "CN" : '帮助'}
              ];
