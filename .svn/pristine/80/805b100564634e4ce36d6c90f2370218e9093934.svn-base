...
<div class="mt10 mb10 posi_re">
	...
	<div class="com_ta2 mt15">
		...
		<div class="com_ta2 ova_sc2 bg_lightgray orgChartItemList" style="height:148px;margin-top:0px;" id="div_org_item_list2_dm" style="display:none;">
			<table class="tb_item_list">
				<colgroup>
					<col width="34" />
					<col width="50%" />
		...
		for (var i = 0; i < param.length; i++) {
			...
			$('.orgChartItemList:visible th:visible').each(function (index, item) {
				...
				if (colName === 'cen') {
					...
					insertRowHtml += '<td class="cen chktd"> <input type="checkbox" name="inp_chk" id="inp_chk_indexOf' + i + '" class="k-checkbox ' + $(item).find(':checkbox').attr('id') + ' checkBox_item chkSel2 chkSel_forEvent " value=\'' + jsonParmData + '\'>	\n<label class="k-checkbox-label chkSel_forEvent bdChk2" 	for="inp_chk_indexOf' + i + '" style=""></label></td>'
				} else {
			...
			insertRowHtml += "<input type='hidden' value='" + jsonParmData2 + "'/></tr>";
		}
		/* 테이블 로우 정보 삽입 */
