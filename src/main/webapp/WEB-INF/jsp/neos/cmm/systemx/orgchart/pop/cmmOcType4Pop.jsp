	...
	defaultInfo.selectMode = "u";
		...
		function callbackOrgChart(data) {
			...
			$.ajax({
				...
				data:{deptSeq:dept, groupSeq:groupSeq, langCode: langCode},
				datatype:"json",			
				success:function(data){
					if(data != null || data !="") {
						$("#selectEmpTable3 tr").remove();
...
//	 			tag += '</td>';
	   ...
	   function fnGroupSearch(){
		   if($("#searchDiv1").val() == "<%=BizboxAMessage.getMessage("TX000000002","그룹명")%>"){
			   ...
			   var tblParam = {};			   
			    ...
			    tblParam.searchDiv = $("#searchDiv1").val();
			    tblParam.txtSearch = $("#txtSearch1").val();
			    tblParam.addrGroupSeq = selectedAddrGroupSeq;
	   ...
	   function InitSearch(treeViewId, searchInputId) {
		...
		function fnSetArrdGroupInfo(e){
			var tblParam = {};
			tblParam.addrGroupSeq = $("#treeview").data("kendoTreeView").dataItem(e.node).group_seq;
						...
						<div class="mt10">
							<div class="clear mb10">
								<div class="option_top">
									<ul>
										<li class="tit_li"><%=BizboxAMessage.getMessage("TX000016028","사용자 목록")%><span id="selectCount1">(0)</span></li>
									</ul>

									<div id="" class="controll_btn p0 fr">
										<button id="" onclick="fnConfirm1();"><%=BizboxAMessage.getMessage("TX000021869","반영")%></button>
									</div>
								</div>
							</div>
