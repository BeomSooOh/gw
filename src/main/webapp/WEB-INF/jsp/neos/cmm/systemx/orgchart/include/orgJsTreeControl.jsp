...
<!-- 부서 선택 -->
<div class="top_box_in orgChartTopBox" id="div_org_search_d" style="display:none;">
	<div id="" class="dod_search posi_re">
		<input type="text" class="txt_search_filter kr" id="text_input"
...
<!-- 회사 조직도 선택 -->
<dl class="orgChartTopBox" id="div_org_search_oc" style="display:none;">
	<dt>회사명</dt>
	<dd>
		<div class="controll_btn p0">
			<input class="fl input_search txt_search_filter kr" id="text_input" type="text" value="" style="width:200px;">
	     	<a href="#" class="fl btn_search"></a>
	     </div>
  	</dd>
</dl>

<!-- 부서 조직도 선택 -->
<dl class="orgChartTopBox" id="div_org_search_od" style="display:none;">
...
<script type="text/javascript">
	...
	function OJTC_documentReadyDf(defaultInfo) {
	...
	function OJTC_documentReadyInit(defaultInfo) {
	...
	function OJTC_documentReady(defaultInfo) {
		OJTC_documentReadyDf(defaultInfo);
		OJTC_documentReadyInit(defaultInfo)
	}
	...
	function OJTC_fnSetSelectedItems(defaultInfo) {
		...
		//그룹형 조직도팝업 체크
		if(typeof(empUniqGroup) != "undefined"){
			data = OJTC_fnGetSelectedItemsEmpUniq(defaultInfo.selectedItems);
			data.empUniqYn = "Y";
		}else{
		...
		data.authPopYn = defaultInfo.authPopYn;
		data.compFilter = defaultInfo.compFilter;		
	...
	function OJTC_fnGetSelectedItemsEmpUniq(selectedStr){
		...
		for (var i = 0; i < items.length; i++) {
			...
			if(temp[4] == 'c'){
				...
				selectedOrg.whereClause_emp += ",'"+temp[3]+"'";
			}
		}
	...
	function OJTC_fnGetEmpDeptList(defaultInfo) {
		...
		var key = defaultInfo.orgGubun + defaultInfo.selectedId + (defaultInfo.isCheckAllDeptEmpShow ? '_1' : '_0') ;
		
		kendo.ui.progress($("body"), true);
		
		if(nodeParsingData.containsKey(key)){
			...
			$.ajax({
				async: true
				, type: "post"
				, url: '<c:url value="/cmm/systemx/GetUserDeptProfileListForDept.do" />'
				, dataType: "json"
				, data: defaultInfo
				, success: function (result) {
	...
	function OJTC_fnSetAllDeptEmpShowEvent(defaultInfo) {
		...
		$("input:checkbox.allDeptEmpShow").click(function(){
			...
			if(defaultInfo.rootCompId == null){
				defaultInfo.rootCompId = defaultInfo.compSeq;
			}
