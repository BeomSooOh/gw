...
//     	}
	...
	function newDept() {
		...
		if(document.getElementById("use_radio_u2").checked){
			if($("#deptType").data("kendoComboBox").value() != "B")
				alert("<%=BizboxAMessage.getMessage("","미사용 부서하위에는 추가 할 수 없습니다.")%>");
			else
				alert("<%=BizboxAMessage.getMessage("","미사용 사업장하위에는 추가 할 수 없습니다.")%>");
			return false;
		}	
	...
	<dl class="dl1">
		...
		<dd style="width: 190px;">
			<input id="txtSearch" class="" type="text" value="" placeholder=""
				style="width: 162px;" placeholder="검색" onkeydown="if(event.keyCode==13){javascript:searchDept();}"> <a href="#"
...
<div class="sub_contents_wrap">
	<div class="posi_re">
		...
		<label class="k-checkbox-label bdChk2" for="bizDisplayYn" >사업장 숨김</label></p>
	</div>
