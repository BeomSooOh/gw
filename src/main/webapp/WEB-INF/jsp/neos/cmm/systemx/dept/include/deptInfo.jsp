...
<script>
	...
	function fnParentDeptList() {
		$("#bs_sel").val('${fn:replace(deptMap.parentDeptName, "'", "\\'")}');
...
<form id="basicForm" name="basicForm"  onsubmit="return false;">
	...
	<div class="tab_div brn" id="tabstrip">
		...
		<p class="fr posi_re text_blue" id="erpUse" style="display:none;top: -26px;right: 35px;"><%=BizboxAMessage.getMessage("TX900000269","ERP 조직도 연동 사용 중")%></p>
		<!-- 기본정보 탭 -->
							...
							<em id="deptDetailType">
								...
								<select id="teamYn" name="teamYn"  class="mr10" style="width:130px" onchange="dllOnChange();"> 
									...
									<option value="Y" <c:if test="${deptMap.deptType == 'T'}">selected</c:if> ><%=BizboxAMessage.getMessage("TX000000639","팀")%></option>
				                  	...
				                  	<option value="E" <c:if test="${deptMap.deptType == 'E'}">selected</c:if> ><%=BizboxAMessage.getMessage("TX900000279","결재전용부서")%></option>
								</select>
							</em>
							<em id="orgDisplayYn">
								<%=BizboxAMessage.getMessage("TX900000446","조직도 미표시")%>
...
<!-- 					</tr> -->
					<tr>
						...
						<td colspan="3"><input type="text" id="deptName" name="deptName" value="${fn:replace(deptMultiMap.deptName, '"', '&quot;')}" style="width:98%"/></td>
					</tr>
					<tr>
						<th><%=BizboxAMessage.getMessage("TX000002790","영&nbsp;&nbsp;어")%></th>
						<td colspan="3"><input class="" type="text" id="deptNameEn" name="deptNameEn" value="${fn:replace(deptMultiMap.deptNameEn, '"', '&quot;')}" style="width:98%"></td>
					</tr>
					<tr>
						<th><%=BizboxAMessage.getMessage("TX000002788","일본어")%></th>
						<td colspan="3"><input class="" type="text" id="deptNameJp" name="deptNameJp" value="${fn:replace(deptMultiMap.deptNameJp, '"', '&quot;')}" style="width:98%"></td>
					</tr>
					<tr>
						<th><%=BizboxAMessage.getMessage("TX000002789","중국어")%></th>
						<td colspan="3"><input class="" type="text" id="deptNameCn" name="deptNameCn" value="${fn:replace(deptMultiMap.deptNameCn, '"', '&quot;')}" style="width:98%"></td>
					</tr>
					<tr id="deptAcronym">
						<c:if test="${deptMap.deptType != 'B'}">
							<th id="nickNameZone"><%=BizboxAMessage.getMessage("TX000000069","부서약칭")%></th>
							<td colspan="4"><input class="" type="text" id="deptNickname" name="deptNickname" value="${fn:replace(deptMultiMap.deptNickname, '"', '&quot;')}" style="width:98%"></td>
						</c:if>
						<c:if test="${deptMap.deptType == 'B'}">
							<th id="nickNameZone"><%=BizboxAMessage.getMessage("TX000000051","사업장약칭")%></th>
							<td colspan="4"><input class="" type="text" id="bizNickname" name="bizNickname" value="${fn:replace(deptMultiMap.deptNickname, '"', '&quot;')}" style="width:98%"></td>
						</c:if>
					</tr>
					<c:if test="${eaType == 'ea'}">
					<tr id="innerReceiveTag" style="display: none;">						
						<th><%=BizboxAMessage.getMessage("TX900000447","대내수신여부")%></th>
						<td colspan="4">		
					...
					<tr>
						...
						<td colspan="2">
							<input type="text" id="zipCode" name="zipCode"  value="${deptMap.zipCode}" style="width:88px;" <c:if test="${ClosedNetworkYn == 'Y'}">placeholder="<%=BizboxAMessage.getMessage("TX000000009","우편번호")%>"</c:if>>
				...
				<table>
					...
					<tr class="comp_type">
						<th colspan="5" class="brtn brrn"><%=BizboxAMessage.getMessage("TX000016224","사업장 추가 정보")%></th>
					</tr>
					<tr class="comp_type">
						...
						<th rowspan="4"><%=BizboxAMessage.getMessage("TX000018684","인감등록")%></th>
						<td rowspan="4">
					...
					<tr class="comp_type">
						...
						<td><input class="" type="text" id="faxNum" name="faxNum" value="${deptMap.faxNum}" style="width:250px;"></td>
					</tr>			
				</table>
			</div>
		</div>
