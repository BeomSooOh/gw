...
<script type="text/javascript">
		...
		function fnGetDataList(){
			...
			var grid = $("#grid").kendoGrid({
				columns: [
						...
						{
							field:"<%=BizboxAMessage.getMessage("TX000010894","입실")%>",
							headerAttributes: {style: "text-align: center; vertical-align:middle;"}, 
							attributes: {style: "text-align: center;"},
							columns: [
								{
									...
									template: function (rowData) {
										...
										if(time != '' && time != null) {
											if(time.length > 4){
												html = html = '<p>' + time.substr(8,2) + ":" + time.substr(10,2) + '</p>';
											}
											else {
												html = '<p>' + time.substr(0,2) + ":" + time.substr(2,2) + '</p>';
											}	
										}
										return html;
									}
								},{
									field:"<%=BizboxAMessage.getMessage("TX000005887","체크")%>",
									width:50,
									columnMenu:true,
									headerAttributes: {style: "text-align: center; vertical-align:middle;"},
									attributes: {style: "text-align: center;"},
									template: '<input type="button" value="확인" class="small_btn" id="btn_in_#: r_no #" style="display: none;" onclick="fnTimeCheck(#: r_no #, #: seq #, 1);"/>'
								}
							]
						},
						{
							field:"<%=BizboxAMessage.getMessage("TX000010893","퇴실")%>",
							headerAttributes: {style: "text-align: center; vertical-align:middle;"}, 
							attributes: {style: "text-align: center;"},
							columns: [
								{
									...
									template: function (rowData) {
										...
										if(time != '' && time != null) {
											if(time.length > 4){
												html = '<p>' + time.substr(8,2) + ":" + time.substr(10,2) + '</p>';
											}
											else {
												html = '<p>' + time.substr(0,2) + ":" + time.substr(2,2) + '</p>';
											}	
									...
									headerAttributes: {style: "text-align: center; vertical-align:middle;"},
									attributes: {style: "text-align: center;"},
									template: '<input type="button" value="확인" class="small_btn" id="btn_out_#: r_no #" style="display: none;" onclick="fnTimeCheck(#: r_no #, #: seq #, 2);"/>'
								}
							]
						},
						{
						...
						{
							field:"<%=BizboxAMessage.getMessage("TX000010892","표찰번호")%>",
							width:70,
					  		headerAttributes: {style: "text-align: center; vertical-align:middle;"}, 
					  		attributes: {style: "text-align: center;"},
							template: '<input type="text" style="width:50px; height:18px;text-align: center;" id="card_no_#: r_no #"/>'
						}
		...
		function btnSetting(){
			...
			for(var i=0;i<gridData.length;i++){
				...
				if((gridData[i].in_time == null || gridData[i].in_time == '') && gridData[i].visit_dt == today){
					document.getElementById("btn_in_"+gridData[i].r_no).style.display = "block";
				}
				
				//방문날짜가 현재 날짜와 같고입실 처리가 되어 있는 경우 퇴실버튼 출력
				if((gridData[i].out_time == null || gridData[i].out_time == "") && gridData[i].visit_dt == today){
					if(gridData[i].in_time != "" && gridData[i].in_time != null){
						document.getElementById("btn_out_"+gridData[i].r_no).style.display = "block";
					}
				}
		...
		function fnTimeCheck(r_no, seq, type) {
	        ...
	        $.ajax({
  				...
  				success:function(data){
  					if(sType == "in")
  						alert("<%=BizboxAMessage.getMessage("TX000010890","입실처리 되었습니다")%>");
  					else
  						alert("<%=BizboxAMessage.getMessage("TX000017929","퇴실처리 되었습니다")%>");
  					fnGetDataList();
  				},error : function(data){
