...
public class EgovLoginLogController {
	...
	public String selectLoginLogInf(@ModelAttribute("searchVO") LoginLog loginLog,
		...
		String searchBgnDe = loginLog.getSearchBgnDe();
		String searchEndDe = loginLog.getSearchEndDe();
		String searchWrd = loginLog.getSearchWrd();
		
		if(EgovStringUtil.isEmpty(searchBgnDe)){
//			searchBgnDe = EgovDateUtil.formatDate(EgovDateUtil.addDay(toDay,  -7),"-");//최근 일주일
			searchBgnDe = EgovDateUtil.formatDate(toDay,"-");
		}
		if(EgovStringUtil.isEmpty(searchEndDe)){
			searchEndDe = EgovDateUtil.formatDate(toDay,"-");
		}
...
//		loginLog.setSearchBgnDe(searchBgnDe);
//		loginLog.setSearchEndDe(searchEndDe);
		loginLog.setSearchWrd(searchWrd);

		HashMap map = (HashMap)loginLogService.selectLoginLogInf(loginLog);
		int totCnt = Integer.parseInt((String)map.get("resultCnt"));
		
		model.addAttribute("resultList", map.get("resultList"));
		model.addAttribute("resultCnt", map.get("resultCnt"));
		model.addAttribute("searchWrd", searchWrd);
