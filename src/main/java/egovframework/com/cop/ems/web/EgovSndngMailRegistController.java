...
public class EgovSndngMailRegistController {
	...
	public String insertSndngMail(final MultipartHttpServletRequest multiRequest,
    	...
    	List<FileVO> sresult = new ArrayList<FileVO>();	// 2012.11 KISA 보안조치
    	String satchFileId = "";
    	final Map<String, MultipartFile> files = multiRequest.getFileMap();
    	if(!files.isEmpty()){
    	 sresult = fileUtil.parseFileInf(files, "MSG_", 0, "", ""); 
    	 satchFileId = fileMngService.insertFileInfs(sresult);  //파일이 생성되고나면 생성된 첨부파일 ID를 리턴한다.
    	...
    	for (int i = 0; i < sresult.size(); i++) {
    		FileVO fileVO = sresult.get(i);
    		orignlFileList = fileVO.getOrignlFileNm();
		}
    	
    	sndngMailVO.setAtchFileId(satchFileId);
